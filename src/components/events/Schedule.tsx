...
const lastElementInColumn = (() => {
  ...
  for (let i = 0; i < 2; i++) {
    for (let j = 0; j < tableContent[i].length; j++)
      if ((tableContent[i][j][1] as number) > 0) ret[i] = j;
  }
  return ret;
})();
...
function getCellStyle(x: number, y: number) {
  if (x === 0 && y === 0) return { ...cellStyles.topLeft, ...cellStyles.common };
      ...
      <>
        ...
        <RainbowDialog open={open} onClose={handleClose}>
          ...
          <DialogContent>
            ...
            <Typography variant="body1" marginTop="3vh">
              由社群和企業展示其領域的專業知識和成果，讓參與者可以實際的深入了解不同社群所擅長的技術，也可以了解企業的產品及研究。當中也有互動和闖關可以讓社群成員和參與者討論不一樣的觀點和經驗，與企業彼此互相交流，了解目前的趨勢及動態。
            </Typography>
          </DialogContent>
        </RainbowDialog>
      </>
